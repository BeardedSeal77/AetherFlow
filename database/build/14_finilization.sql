...
DO $$
DECLARE
    auth_test_record RECORD;
BEGIN
    -- Test authentication function
    SELECT * INTO auth_test_record 
    FROM security.authenticate_user('SYS001', 'admin123');
    
    IF NOT auth_test_record.success THEN
        RAISE EXCEPTION 'Authentication system test failed: %', auth_test_record.message;
    END IF;
    
    RAISE NOTICE 'Authentication system test: PASSED';
    
    -- Test session validation
    IF security.validate_session(auth_test_record.session_token) IS NULL THEN
        RAISE EXCEPTION 'Session validation test failed';
    END IF;
    
    RAISE NOTICE 'Session validation test: PASSED';
    
    -- Cleanup test session
    PERFORM security.logout_user(auth_test_record.session_token);
...
BEGIN
    ...
    SELECT * INTO customer_result 
    FROM core.create_customer_with_contact(
        'Test Customer Ltd', true, 'Test', 'Contact', 
        'test@example.com', '+27111111111', 1
    );
    
    IF NOT customer_result.success THEN
        RAISE EXCEPTION 'Customer creation test failed: %', customer_result.message;
    END IF;
    
    RAISE NOTICE 'Customer creation test: PASSED (ID: %)', customer_result.customer_id;
    
    -- Test interaction creation
    SELECT * INTO interaction_result 
    FROM interactions.create_interaction(
        customer_result.customer_id, customer_result.contact_id, 1, 
        'price_list', 'phone', 'Test interaction'
    );
    
    IF NOT interaction_result.success THEN
        RAISE EXCEPTION 'Interaction creation test failed: %', interaction_result.message;
    END IF;
    
    RAISE NOTICE 'Interaction creation test: PASSED (Ref: %)', interaction_result.reference_number;
    
    -- Test task creation
    SELECT * INTO task_result 
    FROM tasks.create_user_task(
        interaction_result.interaction_id, 1, 'send_price_list',
        'Test task', 'Test task description'
    );
    
    IF NOT task_result.success THEN
        RAISE EXCEPTION 'Task creation test failed: %', task_result.message;
    END IF;
    
    RAISE NOTICE 'Task creation test: PASSED (ID: %)', task_result.task_id;
    
    -- Cleanup test data
    DELETE FROM tasks.user_taskboard WHERE id = task_result.task_id;
    DELETE FROM interactions.interactions WHERE id = interaction_result.interaction_id;
    DELETE FROM core.contacts WHERE id = customer_result.contact_id;
    DELETE FROM core.customers WHERE id = customer_result.customer_id;
    
    RAISE NOTICE 'Business logic validation: PASSED';
END $$;
